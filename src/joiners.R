...
# input <- '/var/folders/tw/tzkjfqd11md5hdjcyjxgmf740000gn/T/file726180705789120480.csv'
# model.out <- '/var/folders/tw/tzkjfqd11md5hdjcyjxgmf740000gn/T/file8865703702117111587.csv'
# rates.out <- '/var/folders/tw/tzkjfqd11md5hdjcyjxgmf740000gn/T/file2591792755455537745.csv'
...
diffs <- df %>% arrange(admission_age, beginning) %>% group_by(admission_age) %>%
  mutate(diff = interval(lag(beginning), beginning) / days(1), n = n()) %>%
  ungroup %>% filter(!is.na(diff) & n >= 3) %>% # We need at least 3 data points for each age to generate 2 diffs
  dplyr::select(admission_age, diff, beginning) %>% mutate(diff = diff + 0.01) %>%
